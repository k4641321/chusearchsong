...
async def 收藏(song,self):
    # 配置文件路径=self.paths.data / "config.json"
...
async def 创建收藏目录(self):
    ...
    with open(self.配置文件路径,"r",encoding="utf-8") as f:
        配置文件=json.load(f)
        logger.info(配置文件)
    保存前提示=toga.InfoDialog("提示", "由于答辩toga外加安卓的限制，文件默认只能保存在 /data/user/0 这样无法访问的路径，请选择其他保存路径")
    await self.main_window.dialog(保存前提示)

    保存提示框=toga.SaveFileDialog("选择",suggested_filename="favorite.json")
    结果= await self.main_window.dialog(保存提示框)
    if 结果:
        logger.info(结果)
        ...
        提示=toga.InfoDialog("成功","成功")
        await self.main_window.dialog(提示)
    else:
        错误=toga.ErrorDialog("错误","未选择路径")
